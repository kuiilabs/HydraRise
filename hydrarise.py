...
def _safe_save(path: Path, obj: dict) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.error("无法弹出错误窗口（可能是无 GUI 环境）")
...
def _run_dry_demo() -> None:
    ...
    logger.info("Dry-run time: %s", now.strftime("%Y-%m-%d %H:%M"))
    logger.info("Current period: %s", engine.current_period(now))
    logger.info("should_drink=%s", engine.should_drink(now, cfg, rec))
    logger.info("should_sit_break=%s", engine.should_sit_break(now, cfg, rec))
    logger.info("should_micro_break=%s", engine.should_micro_break(now, cfg, rec))
    logger.info("risk_level=%s", engine.risk_level(now, cfg, rec))


def main():
    parser = argparse.ArgumentParser(description="HydraRise - 程序员健康提醒助手")
    parser.add_argument("--dry-run", action="store_true", help="无 GUI 演示模式（仅打印提醒状态）")
